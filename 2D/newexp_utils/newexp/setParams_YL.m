...
function nTimeSteps = setParams (inputpath,codepath,listterm,Nx,Ny,Nr)  
  ...
  simTime = 60*t1day; %%% Simulation time  
  ...
  Ly = 75*m1km; %%% Domain size in y  ----> depends on defined variable grid resolution. Better way to do this? 
  H = 2500; %%% Domain size in z 
  g = 9.81; %%% Gravity
  %Omega = 2*pi*366/365/86400;  
  %lat0 = -80; %%% Latitude at southern boundary
  f0 = 1.43e-4; %2*Omega*sind(lat0); %%% Coriolis parameter      
  rho0 = 1027;
  cp = 3974.0; %%% specific heat capacity for the ocean (J/kg/K) ------------- ADDED
  
  %%% Diffusion parameters
  viscAh = 2.5; %%% 
  viscA4 = 0; %%% Biharmonic viscosity ------> does this number even make sense? Also, should it be viscA4W since it'gs NH?
...
%   diffK4Tgrid = 0.2; %%% Grid-dependent biharmonic diffusivity
  ...
  viscAr = 0.01; %%% Vertical viscosity
  ...
  %parm01.addParm('convertFW2Salt',0,PARM_REAL); %%%%%%%%%%%%%% was 0
  %%% equation of state
  ...
  parm01.addParm('debugLevel',3,PARM_INT); %%%%%%%%%%%%%%%%%%%% was -1
...
%   end
  ...
  dy = Ly/Ny*ones(1,Ny); %interp1(NN_vec,dy_vec,1:NN_vec(end),'pchip');
  yy_s = [0 cumsum(dy)];
...
%  parm05.addParm('saltFluxFile','saltFluxFile.bin',PARM_STR);
  ...
  Wsponge = 2*m1km;
  Nsponge = round(Ny - Wsponge/dy(1));
  
  %%% mask nonzero in sponge layer
  rbcs_msk(:,Nsponge:Ny,:) = 1;
...
%   end
  ...
  temp_ref = -1.9; 
  yShelf = 60*m1km/dy(1); %%% need to code this better
