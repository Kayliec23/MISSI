...
function newexp (batch_name,exp_name)
  ...
  runsdir = '/data3/kayliec23/MITgcm_IP/experiments/configurations/';
  %cd(runsdir); cd ../..
...
%  Nr = 100; %%% no. of z-gridpoints  
  ...
  %%% Set-up for Ardbeg, 400 x 400 x 100
   ...
   sNy = 125;      %%% no. of y-gridpoints per tile
   nPx = 1;     %%% no. of processors in x-direction
   nPy = 5;     %%% no. of processors in y-direction
   Nr = 250;    %%% no. of z-gridpoints  
...
%    Nr = 140;    %%% no. of z-gridpoints  
  ...
  Nt = setParams_YL(inputpath,codepath,listterm,Nx,Ny,Nr);
