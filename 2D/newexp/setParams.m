...
function nTimeSteps = setParams (inputpath,codepath,listterm,Nx,Ny,Nr)  
  ...
  Ly = 42*m1km; %%% Domain size in y  ----> depends on defined variable grid resolution. Better way to do this?
  Lsponge = 2*m1km;
  Lshelf = Ly-Lsponge;
  if (use_3D)
    Lx = Ly*m1km; %%% Domain size in x 
  else
...
%   diffK4Tgrid = 0.2; %%% Grid-dependent biharmonic diffusivity
  ...
  parm01.addParm('debugLevel',2,PARM_INT); %%%%%%%%%%%%%%%%%%%% was -1
...
%   parm01.addParm('saltImplVertAdv',true,PARM_BOOL);
  ...
  parm03.addParm('pChkptFreq',1*t1day,PARM_REAL);
...
%   pload = -icetopo;
   ...
   %%% tanh ice topography
  sfactor = 1; % slope factor from reference
%  slope = 0.03*sfactor;
%  yShelf = Lshelf;
  nShelf = Lshelf/dy(1);
  Hshelf = dz(1);
...
%  yshelf = yShelf/2;
%  offset = -3000/sfactor;
%  icetopo = ones(Nx,Ny).*hdiff/2.*(tanh((yy-yshelf-offset)/ywidth + 1)) - Hmax/2 + dz(1);
%  if icetopo(:,nShelf)~=-Hshelf
%      icetopo = icetopo - icetopo(:,nShelf) -dz(1);
%  end
%  icetopo(:,nShelf:Ny) = -Hshelf;

% for same slope with changing domain sizes
  Hice = 500;
  ywidth = (3/8)*Ly*sfactor;
  icetopo = ones(Nx,Ny).*(-Hice + (Hice-dz(1)).*tanh(yy/ywidth));
 % icetopo(:,nShelf:Ny) = -Hshelf;
...
%   [N2 pp_mid] = gsw_Nsquared(SA,CT,pp);
  ...
  Umax = 0.5;  
  Wmax = 0.025;% 0.1  %%% NEEDS TO BE INCREASED FOR DEEP PYCNOCLINES
...
%  parm05.addParm('saltFluxFile','saltFluxFile.bin',PARM_STR);
  ...
  if (use_GGL90)
      ...
      %mxlSurfFlag = 1;
      %GGL90TKEsurfMin = 1e-7; % minimum surface boundary condition
 ...
 %%%%%%%%%%%%%%%%%%%%%  
  ...
  rbcs_msk=zeros(Nx,Ny,Nr); % 2D, for 3D slope_mask=zeros(Nx,Ny,Nr);

  Nsponge = nShelf + 1;
  %%% mask nonzero in sponge layer
  rbcs_msk(:,Nsponge:Ny,2:Nr) = 1;
  %rbcs_msk_vel(:,Nsponge:Ny,:) = 1;
  
%   %%% forcing from below
%   Wforcing = 15*m1km;
%   %WforcingRelax = 2*m1km; % relax to zero area 
%   Nforcing = Wforcing/dy(1);
%   Hforcing = Hmax;
%   NrForcing = Hmax/dz(1);
%   rbcs_msk(:,1:Nforcing,NrForcing) = 1;
...
%   writeDataset(rbcs_msk_vel,fullfile(inputpath,'rbcs_mask_vel.bin'),ieee,prec); 
%   rbcs_parm01.addParm('relaxMaskUFile','rbcs_mask_vel.bin',PARM_STR);  
%   writeDataset(rbcs_msk_vel,fullfile(inputpath,'rbcs_mask_vel.bin'),ieee,prec); 
%   rbcs_parm01.addParm('relaxMaskVFile','rbcs_mask_vel.bin',PARM_STR);
  
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %%%%% RELAXATION TEMPERATURE %%%%%
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  temp_sponge= zeros(Nx,Ny,Nr); 
  salt_sponge= zeros(Nx,Ny,Nr); 
%   temp_force= zeros(Nx,Ny,Nr);
%   salt_force= zeros(Nx,Ny,Nr);
  
  %%% set sponge relaxation values
  t_sponge = repmat(tRef',1,Ny-Nsponge+1,Nx); t_sponge = permute(t_sponge,[3 2 1]);
  temp_sponge(:,Nsponge:Ny,:) = t_sponge;
  s_sponge = repmat(sRef',1,Ny-Nsponge+1,Nx); s_sponge = permute(s_sponge,[3 2 1]);
  salt_sponge(:,Nsponge:Ny,:) = s_sponge;
%   u_sponge = zeros(Nx,Ny,Nr);  
%   v_sponge = zeros(Nx,Ny,Nr);
 % 
%   %%% set forcing relaxation values
%   Tbuoyancy = -4.93e-6; %kg/m^2/s
%   Sbuoyancy = -2.92e-5; %kg/m^2/s
%   
%   Vf = Wforcing*Lx*dz(1); % volume of forcing
%   Mwater = rho0*Vf; % mass of water
%   alphaT = 1.5e-4; % themral expansion coefficient
%   betaS = 7.6e-4; % saline contraction coefficient
%   Tforce = Tbuoyancy.*tauRelaxT/Mwater/alphaT;
%   Sforce = Sbuoyancy*tauRelaxS/Mwater/betaS; 
%   t_force = Tforce*ones(Nx,Nforcing,Nr);
%   temp_force(:,1:Nforcing,:) = tRef(:,1:Nforcing,:) + t_force;
%   s_force = Sforce*ones(Nx,Nforcing,Nr);
%   salt_force(:,1:Nforcing,:) = sRef(:,1:Nforcing,:) + s_force;
%   
%   temp_relax = temp_sponge + temp_force;
%   salt_relax = salt_sponge + salt_force;

temp_relax = temp_sponge;
salt_relax = salt_sponge;

  %%% Save as parameters
  writeDataset(temp_relax,fullfile(inputpath,'temp_relax.bin'),ieee,prec); 
  rbcs_parm01.addParm('relaxTFile','temp_relax.bin',PARM_STR);  
...
%   writeDataset(u_sponge,fullfile(inputpath,'sponge_u.bin'),ieee,prec); 
%   rbcs_parm01.addParm('relaxUFile','sponge.bin',PARM_STR);  
%   writeDataset(v_sponge,fullfile(inputpath,'sponge_v.bin'),ieee,prec); 
%   rbcs_parm01.addParm('relaxVFile','sponge_v.bin',PARM_STR);
...
%       %%%%%%%%%%%%%%%%%%%%%%%
%       
%       %%% need to add a section that changes the number of passive tracer
%       %%% in pkg/ptracers/PTRACERS_SIZE.h
%       
%       %%% To store parameter names and values
%       ptracers_parm01 = parmlist;
%       PTRACERS_PARM = {ptracers_parm01};
%       
%       ptracers_parm01.addParm('usePTRACERS',use_pracers,PARM_BOOL);
%       ptracers_parm01.addParm('PTRACERS_iter0',0,PARM_BOOL);
%       
%       % Tracer 1    
%       ptracer_name = 'T';
%       ptracers_parm01.addParm('PTRACER_name(1)',ptracer_name,PARM_BOOL);
%   
%       
%       
%       
%       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%       %%%%% WRITE THE 'data.ptracers' FILE %%%%%
%       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%       
%       
%       %%% Creates the 'data.ggl90' file
%       write_data_ptracers(inputpath,PTRACERS_PARM,listterm,realfmt);
%   end
%   
...
%     'MXLDEPTH',... %%% ML depth
     ...
     'THETA', ... %%% Temperature
     'SALT', ... %%% Salinity
     'RHOAnoma',... %%% Density anomaly
     'SHIfwFlx'...%%'SHIhtFlx','SHIForcT','SHIForcS', ...
     'UVELTH','VVELTH','WVELTH', ... %%% Temperature fluxes
     'PsiVEL',... %%% horizontal velocity streamfunction
     'ADVr_TH','ADVy_TH', ... %%%%% Advective fluxes of H.B.
     'KPPdiffT','KPPdiffS','KPPhbl','KPPviscA','MXLDEPTH'... %%%% KPP non-local flux of P.T.
  };
...
%   end
  ...
  %packages.addParm('usePTRACERS',use_ptracers,PARM_BOOL);
  ...
  if (use_shelfIce)
    ALL_PARMS = [ALL_PARMS SHELFICE_PARM KPP_PARM];
  else 
    ALL_PARMS = [ALL_PARMS];
  end    
